...
@section('title', 'Arsip Shelter - PawRise Admin')
@section('content')
...
<div class="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-3">
    <div>
        <h3 class="fw-bold mb-0">Arsip Shelter</h3>
        <p class="daftar-subtitle">Data shelter yang dihapus bisa dipulihkan kembali di sini</p>
    </div>
    <a href="{{ route('admin.shelters.index') }}" class="btn-back">
            ...
            @forelse($shelters as $shelter)
                <tr>
                    ...
                    <td style="font-size:.82rem; color:var(--pr-text-muted);">
                        {{ $shelter->deleted_at->format('d M Y, H:i') }}
                    </td>
                    <td class="text-end">
                        <div class="d-flex gap-2 justify-content-end">
                            <form method="POST" action="{{ route('admin.shelters.restore', $shelter->id) }}" class="d-inline">
                                @csrf
                                <button class="hw-icon-btn res" type="submit" title="Pulihkan">
                                    <i class="bi bi-arrow-counterclockwise"></i>
                                </button>
                            </form>
                            <form method="POST" action="{{ route('admin.shelters.force-delete', $shelter->id) }}"
                                  class="d-inline" onsubmit="return confirm('Hapus permanen data shelter ini? Tindakan ini tidak dapat dibatalkan.')">
                                @csrf @method('DELETE')
...
<div class="paw-pagination mt-3 d-flex justify-content-end px-3 pb-3">
    {{ $shelters->links('pagination::bootstrap-4') }}
</div>
@endif
